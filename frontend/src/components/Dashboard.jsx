...
const UploadSlot = ({ slotIndex, currentUser, onComplete }) => {
        ...
        <div className="flex flex-col">
          ...
          {result.videoUrl ? (
            <div className="relative bg-black w-full">
              ...
              <div className={`absolute top-2 left-2 flex items-center gap-1.5 px-2 py-1 rounded text-xs font-sora font-black backdrop-blur-sm ${
                result.safetyStatus === 'SAFE'
                  ? 'bg-neon-green/90 text-black'
                  : 'bg-neon-red/90 text-white'
              }`}>
                {result.safetyStatus === 'SAFE' ? '✓' : '⚠'} {result.safetyStatus}
              </div>
              {result.confidence != null && (
                <div className="absolute top-2 right-2 px-2 py-1 rounded text-xs font-mono-jet text-white bg-black/60 backdrop-blur-sm">
                  {(result.confidence * 100).toFixed(1)}%
                </div>
              )}
            </div>
          ) : (
            /* No video URL — fallback to status banner */
            <div className={`flex items-center gap-3 px-4 py-3 flex-shrink-0 ${
              result.safetyStatus === 'SAFE'
                ? 'bg-neon-green/10 border-b border-neon-green/30'
                : 'bg-neon-red/10 border-b border-neon-red/30'
            }`}>
              <span className={`text-lg ${result.safetyStatus === 'SAFE' ? 'text-neon-green' : 'text-neon-red'}`}>
                {result.safetyStatus === 'SAFE' ? '✓' : '⚠'}
              </span>
              <span className={`font-sora text-sm font-bold ${result.safetyStatus === 'SAFE' ? 'text-neon-green' : 'text-neon-red'}`}>
                {result.safetyStatus}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-3 px-3 py-2 border-t border-white/5 flex-wrap">
            <p className="font-mono-jet text-xs text-slate-600 truncate flex-1 min-w-0">{file?.name}</p>
            {result.safePercent != null && (
              <span className="font-mono-jet text-xs text-neon-green whitespace-nowrap">{result.safePercent.toFixed(1)}% safe</span>
            )}
            {result.unsafePercent != null && (
              <span className="font-mono-jet text-xs text-neon-red whitespace-nowrap">{result.unsafePercent.toFixed(1)}% unsafe</span>
            )}
            {result.videoUrl && (
              <a
                href={`${API_BASE}${result.videoUrl}`}
                download
                className="flex items-center gap-1 font-mono-jet text-xs text-neon-cyan hover:text-white transition-colors whitespace-nowrap"
          ...
          {result.confidence != null && (
            <div className="px-3 pb-2">
              <div className="w-full bg-slate-800/60 rounded-full h-1">
                <div
                  className={`h-1 rounded-full transition-all duration-700 ${result.safetyStatus === 'SAFE' ? 'bg-neon-green' : 'bg-neon-red'}`}
  ...
  return (
    <>
      ...
      {/* Normal card */}
      <div className={`glass-panel rounded-sm flex flex-col border transition-all duration-300 ${borderCls}`}>
        {slotContent}
      </div>
    </>
  );
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (currentUser?.email) fetchQuickStats(currentUser.email);
  }, [currentUser]);

  // Poll stats every 10 seconds so sidebar numbers stay live
  useEffect(() => {
    if (!currentUser?.email) return;
    const email = currentUser.email;
    const id = setInterval(() => fetchQuickStats(email), 10000);
  ...
  const fetchQuickStats = async (email) => {
    const addr = email || currentUser?.email;
    if (!addr) return;
    try {
      const r = await axios.get(`${API_BASE}/videos/stats`, {
        params: { user_email: addr.toLowerCase() },
      });
  ...
  return (
    <div className="fixed inset-0 top-14 flex cyber-grid overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* ── UPLOAD TAB — always mounted, hidden when inactive so slot state survives tab switches ── */}
        <div className={`flex-1 overflow-y-auto p-4 sm:p-5 lg:p-6 ${activeTab === 'upload' ? 'block' : 'hidden'}`}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 auto-rows-auto">
            {[0, 1, 2, 3].map(i => (
              <UploadSlot
                ...
                currentUser={currentUser}
                onComplete={(result) => {
                  fetchQuickStats(currentUser?.email);
                  if (result) {
